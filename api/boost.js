...
module.exports = async (req, res) => {
    ...
    try {
        // Warm-up read: on a fresh serverless invocation, running .transaction()
        // as the very first database operation can sometimes see a stale/empty
        // local cache (null) instead of the real value, causing it to wrongly
        // abort as "insufficient funds". Doing a plain read first ensures the
        // connection has the real, current value before the transaction runs.
        await db.ref(`users/${uid}/coins`).once('value');
