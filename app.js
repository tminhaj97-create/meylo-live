...
// instead of being embedded as giant base64 text blobs directly in the
// Realtime Database — much cheaper/faster to store and load as the app
// grows. Only the resulting download URL gets saved to the user's profile.
function upPhoto(e) {
    ...
    const path = `gallery/${currentUser.uid}/${Date.now()}_${f.name}`;
    storage.ref(path).put(f)
        ...
        .catch(() => showToast("⚠️ Upload failed — try again"));
}
...
let pendingVoiceBlob = null; // holds the just-recorded clip until saved
...
async function toggleVoiceRecording() {
    ...
    try {
        ...
        voiceMediaRecorder.onstop = () => {
            ...
            pendingVoiceBlob = new Blob(voiceChunks, { type: 'audio/webm' });
            const player = document.getElementById('voicePreview');
            player.src = URL.createObjectURL(pendingVoiceBlob); // local preview only — not yet uploaded
...
function deleteVoiceIntro() {
    const oldUrl = currentUser.voiceIntro;
    ...
    if (oldUrl && oldUrl.includes('firebasestorage')) {
        storage.refFromURL(oldUrl).delete().catch(() => {});
    }
}

function delP(idx) {
    const removedUrl = currentUser.gallery[idx];
    ...
    // Best-effort cleanup of the actual file in Storage — non-fatal if it fails.
    if (removedUrl && removedUrl.includes('firebasestorage')) {
        storage.refFromURL(removedUrl).delete().catch(() => {});
    }
}
...
async function saveProf() {
    ...
    if (pendingVoiceBlob) {
        showToast("⏳ Uploading voice intro...");
        try {
            const oldUrl = currentUser.voiceIntro;
            const path = `voice/${currentUser.uid}/${Date.now()}.webm`;
            const snap = await storage.ref(path).put(pendingVoiceBlob);
            currentUser.voiceIntro = await snap.ref.getDownloadURL();
            pendingVoiceBlob = null;
            if (oldUrl && oldUrl.includes('firebasestorage')) {
                storage.refFromURL(oldUrl).delete().catch(() => {});
            }
        } catch (err) {
            showToast("⚠️ Voice upload failed — profile saved without it");
        }
    }
