...
async function renderLikedMe() {
    ...
    try {
        const snap = await db.ref(`likedByLog/${currentUser.uid}`).once('value');
        const likers = snap.val() || {};
        const likerUids = Object.keys(likers).sort((a, b) => likers[b] - likers[a]); // newest first

        if (likerUids.length === 0) {
            grid.innerHTML = '<p style="color:#555;font-size:12px;grid-column:1/-1;text-align:center;padding:20px 0">No one has liked you yet — keep exploring Discover!</p>';
            return;
        }
        ...
        grid.innerHTML = validProfiles.map((p, idx) => `
            <div style="cursor:pointer;border-radius:14px;overflow:hidden;position:relative;aspect-ratio:3/4;background:#111" onclick="openV(window._likedMeProfiles[${idx}])">
                <img src="${escapeHTML(p.avatar)}" style="width:100%;height:100%;object-fit:cover">
                <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(to top,rgba(0,0,0,0.9),transparent);padding:10px">
                    <p style="font-size:12px;font-weight:700;color:#fff">${escapeHTML(p.name ? p.name.split(',')[0] : 'Someone')}</p>
                </div>
            </div>
        `).join('');
    } catch (err) {
