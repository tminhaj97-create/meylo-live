...
async function renderLikedMe() {
    ...
    try {
        const [likedBySnap, dismissedSnap] = await Promise.all([
        ...
        const likerUids = Object.keys(likers)
            ...
            .sort((a, b) => likers[b] - likers[a]); // newest first
        ...
        grid.innerHTML = validProfiles.map((p, idx) => `
            <div style="border-radius:14px;overflow:hidden;position:relative;aspect-ratio:3/4;background:#111">
...
async function activateBoost() {
    ...
    try {
        ...
        const data = await resp.json();
        if (!resp.ok) {
            let msg = data.error || 'Could not activate boost';
            if (data.debug) msg += ' | DEBUG: ' + JSON.stringify(data.debug);
            if (typeof data.serverSeesCoins !== 'undefined') {
                msg += ` | txn saw: ${data.serverSeesCoins} coins, UID: ${data.serverSeesUid}`;
            }
            showToast('⚠️ ' + msg);
            return;
        }
