...
async function unblockUser(idx) {
    const p = window._blockedUsersList[idx];
    if (!p || !currentUser || !currentUser.uid) return;
    try {
        await db.ref(`blocks/${currentUser.uid}/${p.id}`).remove();
        await db.ref(`blockedBy/${p.id}/${currentUser.uid}`).remove();
        showToast('✅ Unblocked ' + (p.name ? p.name.split(',')[0] : 'user'));
        openBlockedUsersModal(); // refresh the list
    } catch (err) {
        showToast('⚠️ Could not unblock — try again');
    }
}

async function openBlockedUsersModal() {
    history.pushState({ modal: 'blockedUsers' }, '');
    openMo('blockedUsersMo');
    const list = document.getElementById('blockedUsersList');
    list.innerHTML = '<p style="color:#555;font-size:12px;text-align:center;padding:10px 0">Loading...</p>';
    ...
    try {
        ...
        if (blockedIds.length === 0) {
            list.innerHTML = '<p style="color:#555;font-size:12px;text-align:center;padding:10px 0">You haven\'t blocked anyone.</p>';
            return;
        }
        ...
        const profiles = await Promise.all(blockedIds.map(async uid => {
            ...
            const p = pSnap.val();
            return p || { id: uid, name: 'Deleted user' }; // profile may no longer exist
        }));
        window._blockedUsersList = profiles;

        list.innerHTML = profiles.map((p, idx) => `
            <div style="display:flex;align-items:center;gap:10px;padding:8px;background:#181818;border-radius:12px">
                <img src="${escapeHTML(p.avatar) || ''}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;background:#333">
                <span style="flex:1;font-size:13px">${escapeHTML(p.name ? p.name.split(',')[0] : 'Someone')}</span>
                <button onclick="unblockUser(${idx})" class="btn-secondary" style="padding:6px 14px;font-size:11px">Unblock</button>
            </div>
        `).join('');
    } catch (err) {
        list.innerHTML = `<p style="color:#f44336;font-size:11px;text-align:center;padding:10px 0">Error: ${escapeHTML(err.message)}</p>`;
    }
}
...
function loadCard() {
    ...
    <div class="portal-swipe-card" id="activeSwipeCard">
        ...
        <button onclick="openV('${escapeHTML(p.id)}')" style="position:absolute;top:12px;right:12px;width:34px;height:34px;border-radius:50%;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.3);color:#fff;font-size:14px;z-index:5" title="View Profile"><i class="fas fa-info"></i></button>
...
function swipeR() {
    if (dI < onlineUsers.length) {
        ...
        if (currentUser && currentUser.uid && p.id) {
            ...
            // Daily counter (for the "Like 5 Profiles" task) — one entry per target per day.
            db.ref(`likesLog/${currentUser.uid}/${today}/${p.id}`).set(true).catch(() => {});
            // Permanent reverse index — lets p see, under their OWN account,
            // that we liked them (this is what powers "Liked Me").
            db.ref(`likedByLog/${p.id}/${currentUser.uid}`).set(Date.now()).catch(err => showToast('⚠️ Like save failed: ' + err.message));

            // Check if it's a real mutual match: did THEY already like US?
            // (We can only read our own likedByLog, which is exactly what we need here.)
            db.ref(`likedByLog/${currentUser.uid}/${p.id}`).once('value').then(snap => {
                if (snap.exists()) {
                    showToast("🎉 It's a Match with " + p.name.split(',')[0] + "!");
                } else {
                    showToast("💖 Liked " + p.name.split(',')[0] + "!");
                }
            });
        } else {
            showToast("💖 Liked " + p.name.split(',')[0] + "!");
...
async function openSuperLikeModal() {
    if (dI >= onlineUsers.length) return;
    const p = onlineUsers[dI];
    if (!currentUser || !currentUser.uid || !p.id) { showToast('⚠️ Please log in first'); return; }
    ...
    try {
        ...
        const resp = await fetch('/api/boost', {
            ...
            body: JSON.stringify({ action: 'superlike', targetUid: p.id })
        });
        const data = await resp.json();

        if (!resp.ok) {
            showToast('⚠️ ' + (data.error || 'Could not send Super Like'));
            return;
        }
        ...
        // Still record the normal daily-task-counted like too.
        const today = new Date().toISOString().slice(0, 10);
        db.ref(`likesLog/${currentUser.uid}/${today}/${p.id}`).set(true).catch(() => {});

        showToast('⭐ Super Like sent to ' + p.name.split(',')[0] + '!');
        dI++; loadCard();
        selChat(p);
    } catch (err) {
...
function switchV(v, btn) {
    ...
    closeDD();
    if (v === 'likedMe') renderLikedMe();
}

// Reads who has liked the current user (via the likedByLog reverse index,
// written by swipeR() on the liker's side) and shows their profiles —
// tapping one opens the full visit-profile view.
async function renderLikedMe() {
    const grid = document.getElementById('likedGrid');
    if (!currentUser || !currentUser.uid) return;
    grid.innerHTML = '<p style="color:#555;font-size:12px;grid-column:1/-1;text-align:center;padding:20px 0">Loading...</p>';

    try {
        const [likedBySnap, dismissedSnap, superSnap] = await Promise.all([
            db.ref(`likedByLog/${currentUser.uid}`).once('value'),
            db.ref(`likedMeDismissed/${currentUser.uid}`).once('value'),
            db.ref(`superLikes/${currentUser.uid}`).once('value')
        ]);
        const likers = likedBySnap.val() || {};
        const dismissed = dismissedSnap.val() || {};
        const superLikers = superSnap.val() || {};
        const likerUids = Object.keys(likers)
            .filter(uid => !dismissed[uid]) // already liked-back or passed — don't show again
            .sort((a, b) => {
                const aSuper = superLikers[a] ? 1 : 0;
                const bSuper = superLikers[b] ? 1 : 0;
                if (aSuper !== bSuper) return bSuper - aSuper; // super likes first
                return likers[b] - likers[a]; // then newest first
            });

        if (likerUids.length === 0) {
            grid.innerHTML = '<p style="color:#555;font-size:12px;grid-column:1/-1;text-align:center;padding:20px 0">No one new has liked you yet — keep exploring Discover!</p>';
            return;
        }

        const profiles = await Promise.all(likerUids.map(async uid => {
            const pSnap = await db.ref(`users/${uid}`).once('value');
            const p = pSnap.val();
            if (p) p._isSuperLike = !!superLikers[uid];
            return p;
        }));
        const validProfiles = profiles.filter(p => p);
        window._likedMeProfiles = validProfiles; // temp store so onclick can reference by index safely, without embedding raw data in HTML attributes

        grid.innerHTML = validProfiles.map((p, idx) => `
            <div style="border-radius:14px;overflow:hidden;position:relative;aspect-ratio:3/4;background:#111;${p._isSuperLike ? 'box-shadow:0 0 0 2px var(--vblue)' : ''}">
                ${p._isSuperLike ? '<div style="position:absolute;top:8px;left:8px;background:var(--vblue);color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:10px;z-index:2"><i class="fas fa-star"></i> Super Like</div>' : ''}
                <img src="${escapeHTML(p.avatar)}" style="width:100%;height:100%;object-fit:cover;cursor:pointer" onclick="openV(window._likedMeProfiles[${idx}])">
                <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(to top,rgba(0,0,0,0.9),transparent);padding:10px 8px 8px">
                    <p style="font-size:12px;font-weight:700;color:#fff;margin-bottom:6px">${escapeHTML(p.name ? p.name.split(',')[0] : 'Someone')}</p>
                    <div style="display:flex;gap:6px">
                        <button onclick="event.stopPropagation();passOnLikedMe(${idx})" style="flex:1;background:rgba(244,67,54,0.15);border:1px solid #f44336;color:#f44336;border-radius:10px;padding:6px 0;font-size:13px"><i class="fas fa-times"></i></button>
                        <button onclick="event.stopPropagation();likeBackFromLikedMe(${idx})" style="flex:1;background:rgba(76,175,80,0.15);border:1px solid #4caf50;color:#4caf50;border-radius:10px;padding:6px 0;font-size:13px"><i class="fas fa-heart"></i></button>
                    </div>
                </div>
            </div>
        `).join('');
    } catch (err) {
        grid.innerHTML = `<p style="color:#f44336;font-size:11px;grid-column:1/-1;text-align:center;padding:20px 10px">Error: ${escapeHTML(err.message)}</p>`;
    }
}

// Liking back someone who's already in this list is automatically a real
// mutual match — they liked us first (that's why they're here), and now we
// like them too, so no separate "check for mutual" step is needed.
async function likeBackFromLikedMe(idx) {
    const p = window._likedMeProfiles[idx];
    if (!p || !currentUser || !currentUser.uid) return;

    const today = new Date().toISOString().slice(0, 10);
    db.ref(`likesLog/${currentUser.uid}/${today}/${p.id}`).set(true).catch(() => {});
    db.ref(`likedByLog/${p.id}/${currentUser.uid}`).set(Date.now()).catch(() => {});
    await db.ref(`likedMeDismissed/${currentUser.uid}/${p.id}`).set(true).catch(() => {});

    showToast("🎉 It's a Match with " + (p.name ? p.name.split(',')[0] : 'them') + "!");
    renderLikedMe(); // refresh the list without this person
    switchV('messages', document.getElementById('bbtn-messages'));
    selChat(p);
}

async function passOnLikedMe(idx) {
    const p = window._likedMeProfiles[idx];
    if (!p || !currentUser || !currentUser.uid) return;
    await db.ref(`likedMeDismissed/${currentUser.uid}/${p.id}`).set(true).catch(() => {});
    showToast('Passed');
...
function openV(idOrProfile) {
    history.pushState({ modal: 'visit' }, '');
    const p = typeof idOrProfile === 'object' ? idOrProfile : onlineUsers.find(x => x.id === idOrProfile);
    if (!p) { showToast('⚠️ Profile not available'); return; }
...
async function activateBoost() {
    ...
    try {
        ...
        const data = await resp.json();
        if (!resp.ok) {
            showToast('⚠️ ' + (data.error || 'Could not activate boost'));
            return;
        }
